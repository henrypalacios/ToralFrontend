...
folder are mapped to the application's root namespace, 'ToralVirtual'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ToralVirtual"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ToralVirtual' in the same way
as `"ToralVirtual/sass/src"`.
